...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
    ...
    do {
        ...
        // 调用 get_pte 函数获取源进程地址空间中地址 start 对应的页表项指针 ptep
        pte_t *ptep = get_pte(from, start, 0), *nptep;
        if (ptep == NULL) {
            //如果获取页表项失败
            //将 start 向下舍入到下一个页面的起始地址，并继续下一轮循环
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
        ...
        if (*ptep & PTE_V) {
            //调用 get_pte 函数获取目标进程地址空间中地址 start 对应的页表项指针 nptep
            ...
            // get page from ptep
            // 调用 pte2page 函数将页表项转换为对应的物理页面结构体指针 page
...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    ...
    if (*ptep & PTE_V) {
        ...
        //如果当前页表项已经映射到另一个物理页面
        //减少该物理页面的引用计数
        //否则调用 page_remove_pte 函数从页表中移除当前页表项。
        if (p == page) {
            page_ref_dec(page);
        } else {
            page_remove_pte(pgdir, la, ptep);
