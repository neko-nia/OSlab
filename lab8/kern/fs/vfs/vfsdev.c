 ...
 */
int
//vfs_mount搜索整个设备链表，查找需要挂在文件系统的设备。
vfs_mount(const char *devname, int (*mountfunc)(struct device *dev, struct fs **fs_store)) {
    int ret;
    lock_vdev_list();
    vfs_dev_t *vdev;
    //调用 find_mount 函数来查找设备
    if ((ret = find_mount(devname, &vdev)) != 0) {
        goto out;
    }
    // 如果 vdev 的文件系统指针不为空，表示设备已经被挂载
    if (vdev->fs != NULL) {
        ret = -E_BUSY;
        goto out;
    }
    //确保 vdev 的设备名称不为空且可挂载
    assert(vdev->devname != NULL && vdev->mountable);
    //通过 vop_info 函数获取 vdev 的设备节点信息
    //并将其转换为设备结构体，保存在 dev 中
    struct device *dev = vop_info(vdev->devnode, device);
    //调用传入的 mountfunc 函数来执行挂载操作
