 ...
 * On-disk superblock
 */
// 第 0 个块（4K）是超级块（superblock），它包含了关于文件系统的所有关键参数
// 当计算机被启动或文件系统被首次接触时，超级块的内容就会被装入内存
struct sfs_super {
    // 内核通过magic来检查磁盘镜像是否是合法的 SFS img
...
// 在超级块之后，第1个块放了 root-dir 的 inode，用来记录根目录的相关信息
// 是 SFS 文件系统的根结点,通过这个 root-dir 的 inode 信息就可以定位并查找到根目录下的所有文件信息
// 从第 2 个块开始，根据 SFS 中所有块的数量，用 1 个 bit 来表示一个块的占用和未被占用的情况
// 这个区域称为 SFS 的 freemap 区域，这将占用若干个块空间(bitmap.c/h)
