...
sfs_do_mount(struct device *dev, struct fs **fs_store) {
    ...
    struct fs *fs;
    //分配一个 struct fs 结构体，并将其赋给 fs 指针
    if ((fs = alloc_fs(sfs)) == NULL) {
        return -E_NO_MEM;
    }
    //通过 fsop_info 函数获取文件系统相关的信息
    struct sfs_fs *sfs = fsop_info(fs, sfs);
    sfs->dev = dev;
    ...
    void *sfs_buffer;
    //分配一个大小为 SFS_BLKSIZE 的内存块
    //并将其赋给 sfs->sfs_buffer 和 sfs_buffer
    if ((sfs->sfs_buffer = sfs_buffer = kmalloc(SFS_BLKSIZE)) == NULL) {
        goto failed_cleanup_fs;
    }

    /* load and check superblock */
    //调用 sfs_init_read 函数从设备中读取超级块信息到 sfs_buffer 中
