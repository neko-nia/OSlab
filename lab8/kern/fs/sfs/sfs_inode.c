 ...
 */
static int
//函数以“/”为分割符，从左至右逐一分解path获得各个子目录和最终文件对应的inode节点
sfs_lookup(struct inode *node, char *path, struct inode **node_store) {
    ...
    //调用sfs_lookup_once函数获得子目录对应的inode节点subnode
    
    int ret = sfs_lookup_once(sfs, sin, path, &subnode, NULL);

    //然后循环进一步调用sfs_lookup_once查找子目录下的文件所对应的inode节点
    //当无法分解path后，就意味着找到了对应的inode节点，就可顺利返回了
    vop_ref_dec(node);
    if (ret != 0) {
        return ret;
    }
